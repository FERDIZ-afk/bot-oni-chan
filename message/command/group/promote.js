...
module.exports = {
	...
	async run({ msg, fdz }, { q, prefix }) {
	  try{
		...
		if (!add) return msg.reply("Example: " + prefix + "promote 62728288");
		...
		let participant = await fdz.groupParticipantsUpdate(msg.chat, chunk, "promote").then((res) => msg.reply(util.format(res))).catch((err) => msg.reply(util.format(err)))
	  } catch (err) {
	}
